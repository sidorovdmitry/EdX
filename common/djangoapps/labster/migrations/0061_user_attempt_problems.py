...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        now = datetime.datetime(2014, 12, 15, 0, 0, 0, tzinfo=timezone.utc)
        user_attempts = orm.UserAttempt.objects.filter(lab_proxy__lab_id=35, created_at__lte=now)
        quiz_ids = ["Cyto-{}-Post".format(i) for i in range(11, 41)]
        problems = orm.Problem.objects.filter(is_active=True, element_id__in=quiz_ids)
        for ua in user_attempts:
            ua.problems.add(*problems)
