...
def get_problems_count(user_attempt, orm):
    return orm.Problem.objects.filter(
        is_active=True,
        no_score=False,
        quiz_block__lab=user_attempt.lab_proxy.lab,
    ).count()


def get_score(user_attempt, orm):
    user_answers = orm.UserAnswer.objects.filter(
    ...
    problems_count = get_problems_count(user_attempt, orm)
    if problems_count == 0:
        return None

    score = 10.0 * float(score) / float(problems_count)
    return score
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ua in orm.UserAttempt.objects.filter(score=None):
            score = get_score(ua, orm)
            ua.score = score
            ua.save()
