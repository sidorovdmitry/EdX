...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for problem in orm.Problem.objects.all():
            problem.sentence = problem.sentence.strip()
        ...
        for answer in orm.Answer.objects.all():
            ...
            answer.hashed_text = get_hashed_text(answer.text)
            answer.save()

        for ua in orm.UserAnswer.objects.all():
            ua.question = ua.question.strip()
            ua.answer_string = ua.answer_string.strip()
            ua.correct_answer = ua.correct_answer.strip()
            ua.is_correct = ua.answer_string in ua.correct_answer.split(':::')
            ua.save()
