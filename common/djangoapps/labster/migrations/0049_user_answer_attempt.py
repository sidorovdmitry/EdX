...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        attempts = orm.UserAttempt.objects.order_by('-created_at')
        for attempt in attempts:

            answers = orm.UserAnswer.objects.filter(
                user=attempt.user,
                lab_proxy=attempt.lab_proxy,
                attempt__isnull=True
            ).exclude(quiz_id='').order_by('-created_at')

            quiz_ids = []
            for answer in answers:
                if answer.quiz_id in quiz_ids:
                    break
