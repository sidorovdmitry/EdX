...
def check_completed(orm, user_attempt):
    problem_quiz_ids = orm.Problem.objects.filter(
        is_active=True,
        quiz_block__lab=user_attempt.lab_proxy.lab).values_list('element_id', flat=True)

    answer_quiz_ids = orm.UserAnswer.objects.filter(
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for user_attempt in orm.UserAttempt.objects.filter(is_completed=False):
            user_attempt.is_completed = check_completed(orm, user_attempt)
            user_attempt.save()
