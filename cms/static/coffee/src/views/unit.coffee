...
($, ui, gettext, Backbone, NotificationView, PromptView, ModuleEditView, ModuleModel) ->
  ...
  class UnitEditView.NameEdit extends Backbone.View
    ...
    saveLab: =>
      ...
      if has_quizblocks == "0"
        quizblock_inputs = @$(".lab-" + metadata.lab_id + "-quizblock")
        ...
        if quizblock_inputs.length > 0
          for input in quizblock_inputs
            qb_el = @$(input)

            slug = qb_el.data("slug")
            description = qb_el.data("description")
            create_quizblock_header slug, description

            lab_id = qb_el.data("lab-id")
            quizblock_id = qb_el.data("quizblock")
            textareas = @$(".lab-" + lab_id + "-textarea-quizblock-" + quizblock_id)

            if textareas.length > 0
              for textarea in textareas
                p_el = @$(textarea)
                problem_data = p_el.val()
